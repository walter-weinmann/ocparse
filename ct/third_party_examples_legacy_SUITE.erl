%%%-------------------------------------------------------------------
%%% File        : third_party_examples_legacy_SUITE.erl
%%% Description : Test Suite covering examples from thirs parties.
%%%
%%% Created     : 04.2016
%%%-------------------------------------------------------------------
-module(third_party_examples_legacy_SUITE).
...
groups() ->
    [
        {neo4j, [], [
            ...
            test_neo4j_5_6_compatibility,
            test_neo4j_6_2_expressions,
...
%           test_neo4j_7_8_call,                                                % not supported by openCypher
            ...
            test_neo4j_8_4_start,
            test_neo4j_8_5_aggregation,
            test_neo4j_8_6_load_csv,
            test_neo4j_9_1_create,
            test_neo4j_9_2_merge,
            test_neo4j_9_3_set,
            test_neo4j_9_4_delete,
            test_neo4j_9_5_remove,
            test_neo4j_9_6_foreach,
            test_neo4j_9_7_create_unique,
            test_neo4j_9_8_importing_csv,
...
test_neo4j_4_5_constraints(_Config) ->
    Cypher_01 = "CREATE CONSTRAINT ON (movie:Movie) ASSERT movie.title IS UNIQUE",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "CREATE INDEX ON :Actor(name)",
    octest_legacy:ct_string(Cypher_02),
...
test_neo4j_5_5_parameters(_Config) ->
    ...
    Cypher_10 = "MATCH (n)
                 ...
                 RETURN n.name",
    octest_legacy:ct_string(Cypher_10),
    Cypher_11 = "START n=node:people(name = { value })
                 RETURN n",
    octest_legacy:ct_string(Cypher_11),
    Cypher_12 = "START n=node:people({ query })
                 RETURN n",
    octest_legacy:ct_string(Cypher_12).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 5.6 Compatibility
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

test_neo4j_5_6_compatibility(_Config) ->
    Cypher_01 = "CYPHER 2.2
...
test_neo4j_6_2_expressions(_Config) ->
    Cypher_01 = "MATCH (n)
                 RETURN
                 CASE n.eyes
                 WHEN 'blue'
...
test_neo4j_6_7_lists(_Config) ->
    ...
    octest_legacy:ct_string(Cypher_14).
...
%   octest_legacy:ct_string(Cypher_14),                                                % not supported by openCypher ???
    ...
    octest_legacy:ct_string(Cypher_15),
    Cypher_16 = "MATCH (wallstreet:Movie { title:'Wall Street' })-[*0..1]-(x)
                 RETURN x",
    octest_legacy:ct_string(Cypher_16),
    Cypher_17 = "MATCH p =(michael { name:'Michael Douglas' })-->()
                 RETURN p",
    octest_legacy:ct_string(Cypher_17),
    Cypher_18 = "MATCH (a)-[r]-(b)
                 WHERE id(r)= 0
                 RETURN a,b",
    octest_legacy:ct_string(Cypher_18),
    ...
    Cypher_22 = "MATCH (n)
                 WHERE id(n)= 0
                 RETURN n",
    octest_legacy:ct_string(Cypher_22),
    Cypher_23 = "MATCH ()-[r]->()
                 WHERE id(r)= 0
                 RETURN r",
    octest_legacy:ct_string(Cypher_23),
    Cypher_24 = "MATCH (n)
                 WHERE id(n) IN [0,3,5]
                 RETURN n",
    octest_legacy:ct_string(Cypher_24).
...
test_neo4j_8_4_start(_Config) ->
    Cypher_01 = "START n=node:nodes(name = \"A\")
                 RETURN n",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "START r=relationship:rels(name = \"Andrés\")
                 RETURN r",
    octest_legacy:ct_string(Cypher_02),
    Cypher_03 = "START n=node:nodes(\"name:A\")
                 RETURN n",
    octest_legacy:ct_string(Cypher_03).
...
test_neo4j_8_6_load_csv(_Config) ->
    Cypher_01 = "LOAD CSV FROM 'http://neo4j.com/docs/3.0.3/csv/artists.csv' AS line
                 CREATE (:Artist { name: line[1], year: toInt(line[2])})",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "LOAD CSV WITH HEADERS FROM 'http://neo4j.com/docs/3.0.3/csv/artists-with-headers.csv' AS line
                 CREATE (:Artist { name: line.Name, year: toInt(line.Year)})",
    octest_legacy:ct_string(Cypher_02),
    Cypher_03 = "LOAD CSV FROM 'http://neo4j.com/docs/3.0.3/csv/artists-fieldterminator.csv' AS line FIELDTERMINATOR
                   ';'
                 CREATE (:Artist { name: line[1], year: toInt(line[2])})",
    octest_legacy:ct_string(Cypher_03),
    Cypher_04 = "USING PERIODIC COMMIT
...
test_neo4j_9_7_create_unique(_Config) ->
    Cypher_01 = "MATCH (root { name: 'root' })
                 CREATE UNIQUE (root)-[:LOVES]-(someone)
                 RETURN someone",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "MATCH (root { name: 'root' })
                 CREATE UNIQUE (root)-[:X]-(leaf { name:'D' })
                 RETURN leaf",
    octest_legacy:ct_string(Cypher_02),
    Cypher_03 = "MATCH (a { name: 'A' })
                 CREATE UNIQUE (a)-[:KNOWS]-(c:blue)
                 RETURN c",
    octest_legacy:ct_string(Cypher_03),
    Cypher_04 = "MATCH (lft { name: 'A' }),(rgt)
                 WHERE rgt.name IN ['B', 'C']
                 CREATE UNIQUE (lft)-[r:KNOWS]->(rgt)
                 RETURN r",
    octest_legacy:ct_string(Cypher_04),
    Cypher_05 = "MATCH (root { name: 'root' })
                 CREATE UNIQUE (root)-[r:X { since:'forever' }]-()
                 RETURN r",
    octest_legacy:ct_string(Cypher_05),
    Cypher_06 = "MATCH (root { name: 'root' })
...
test_neo4j_9_8_importing_csv(_Config) ->
    Cypher_01 = "LOAD CSV WITH HEADERS FROM \"http://neo4j.com/docs/3.0.1/csv/import/persons.csv\" AS csvLine
                 CREATE (p:Person { id: toInt(csvLine.id), name: csvLine.name })",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "CREATE INDEX ON :Country(name)",
    octest_legacy:ct_string(Cypher_02),
    Cypher_03 = "LOAD CSV WITH HEADERS FROM \"http://neo4j.com/docs/3.0.1/csv/import/movies.csv\" AS csvLine
                 MERGE (country:Country { name: csvLine.country })
                 CREATE (movie:Movie { id: toInt(csvLine.id), title: csvLine.title, year:toInt(csvLine.year)})
                 CREATE (movie)-[:MADE_IN]->(country)",
    octest_legacy:ct_string(Cypher_03),
    Cypher_04 = "CREATE CONSTRAINT ON (person:Person) ASSERT person.id IS UNIQUE",
    octest_legacy:ct_string(Cypher_04),
...
test_neo4j_11_1_indexes(_Config) ->
    Cypher_01 = "CREATE INDEX ON :Person(name)",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "DROP INDEX ON :Person(name)",
    octest_legacy:ct_string(Cypher_02),
...
test_neo4j_11_2_constraints(_Config) ->
    Cypher_01 = "CREATE CONSTRAINT ON (book:Book) ASSERT book.isbn IS UNIQUE",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "DROP CONSTRAINT ON (book:Book) ASSERT book.isbn IS UNIQUE",
    octest_legacy:ct_string(Cypher_02),
    ...
    octest_legacy:ct_string(Cypher_04),
    Cypher_05 = "CREATE CONSTRAINT ON (book:Book) ASSERT book.isbn IS UNIQUE",
    octest_legacy:ct_string(Cypher_05),
    Cypher_06 = "CREATE CONSTRAINT ON (book:Book) ASSERT exists(book.isbn)",
    octest_legacy:ct_string(Cypher_06),
    Cypher_07 = "DROP CONSTRAINT ON (book:Book) ASSERT exists(book.isbn)",
    octest_legacy:ct_string(Cypher_07),
    ...
    Cypher_10 = "MATCH (book:Book { title: 'Graph Databases' })
                 REMOVE book.isbn",
    octest_legacy:ct_string(Cypher_10),
    Cypher_11 = "CREATE CONSTRAINT ON (book:Book) ASSERT exists(book.isbn)",
    octest_legacy:ct_string(Cypher_11),
    Cypher_12 = "CREATE CONSTRAINT ON ()-[like:LIKED]-() ASSERT exists(like.day)",
    octest_legacy:ct_string(Cypher_12),
    Cypher_13 = "DROP CONSTRAINT ON ()-[like:LIKED]-() ASSERT exists(like.day)",
    octest_legacy:ct_string(Cypher_13),
    ...
    Cypher_16 = "MATCH (user:User)-[like:LIKED]->(book:Book)
                 REMOVE like.day",
    octest_legacy:ct_string(Cypher_16),
    Cypher_17 = "CREATE CONSTRAINT ON ()-[like:LIKED]-() ASSERT exists(like.day)",
    octest_legacy:ct_string(Cypher_17).
...
test_neo4j_12_3_basic_query_tuning(_Config) ->
    Cypher_01 = "LOAD CSV WITH HEADERS FROM \"http://neo4j.com/docs/3.0.1/csv/query-tuning/movies.csv\" AS line
                 MERGE (m:Movie { title:line.title })
                 ON CREATE SET m.released = toInt(line.released), m.tagline = line.tagline",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "LOAD CSV WITH HEADERS FROM 'http://neo4j.com/docs/3.0.1/csv/query-tuning/actors.csv' AS line
                 MATCH (m:Movie { title:line.title })
                 MERGE (p:Person { name:line.name })
                 ON CREATE SET p.born = toInt(line.born)
                 MERGE (p)-[:ACTED_IN { roles:split(line.roles,\";\")}]->(m)",
    octest_legacy:ct_string(Cypher_02),
    Cypher_03 = "LOAD CSV WITH HEADERS FROM 'http://neo4j.com/docs/3.0.1/csv/query-tuning/directors.csv' AS line
                 MATCH (m:Movie { title:line.title })
                 MERGE (p:Person { name:line.name })
                 ON CREATE SET p.born = toInt(line.born)
                 MERGE (p)-[:DIRECTED]->(m)",
    ...
    Cypher_07 = "PROFILE
                 ...
                 RETURN p",
    octest_legacy:ct_string(Cypher_07),
    Cypher_08 = "CREATE INDEX ON :Person(name)",
    octest_legacy:ct_string(Cypher_08),
...
test_neo4j_12_4_using(_Config) ->
    Cypher_01 = "MATCH (liskov:Scientist { name:'Liskov' })-[:KNOWS]->(wing:Scientist)-[:RESEARCHED]->(cs:Science {
                 ...
                 RETURN 1 AS column",
    octest_legacy:ct_string(Cypher_01),
    Cypher_02 = "MATCH (liskov:Scientist { name:'Liskov' })-[:KNOWS]->(wing:Scientist)-[:RESEARCHED]->(cs:Science {
                 name:'Computer Science' })<-[:RESEARCHED]-(conway:Scientist { name: 'Conway' })
                 USING INDEX liskov:Scientist(name)
                 RETURN liskov.born AS column",
    octest_legacy:ct_string(Cypher_02),
    Cypher_03 = "MATCH (liskov:Scientist { name:'Liskov' })-[:KNOWS]->(wing:Scientist)-[:RESEARCHED]->(cs:Science {
                 name:'Computer Science' })<-[:RESEARCHED]-(conway:Scientist { name: 'Conway' })
                 USING INDEX liskov:Scientist(name)
                 USING INDEX conway:Scientist(name)
                 RETURN liskov.born AS column",
    octest_legacy:ct_string(Cypher_03),
    Cypher_04 = "MATCH (s:Scientist)
                 USING SCAN s:Scientist
                 WHERE s.born < 1939
                 RETURN s.born AS column",
    octest_legacy:ct_string(Cypher_04),
    Cypher_05 = "MATCH (liskov:Scientist { name:'Liskov' })-[:KNOWS]->(wing:Scientist)-[:RESEARCHED]->(cs:Science {
                 name:'Computer Science' })<-[:RESEARCHED]-(conway:Scientist { name: 'Conway' })
                 USING INDEX liskov:Scientist(name)
                 USING INDEX conway:Scientist(name)
                 USING JOIN ON wing
    ...
    octest_legacy:ct_string(Cypher_05),
    Cypher_06 = "MATCH (liskov:Scientist { name:'Liskov' })-[:KNOWS]->(wing:Scientist { name:'Wing' })-[:RESEARCHED]-
...
test_neo4j_13_5_update_operators(_Config) ->
    Cypher_01 = "CREATE CONSTRAINT ON (c:Country) ASSERT c.name IS UNIQUE",
    octest_legacy:ct_string(Cypher_01),
