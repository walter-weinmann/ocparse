%%%-------------------------------------------------------------------
%%% File        : octest_legacy.erl
%%% Description : Test helper functions.
%%%
%%% Created     : 06.2016
%%%-------------------------------------------------------------------
-module(octest_legacy).
...
ct_string(Test) ->
    ...
    case ocparse_legacy:parsetree_with_tokens(Test) of
        {ok, {ParseTree, _Tokens}} ->
            ...
            %% Test TopDown
            %% --------------------------------------------------------
            NCypherTD = case ocparse_legacy:parsetree_to_string_td(ParseTree) of
                            {error, ErrorTD} ->
                                throw({error, ErrorTD});
                            NSTD ->
            ...
            {ok, {NPTreeTD, _NToksTD}}
                = try
                      {ok, {NPTTD, NTTD}} = ocparse_legacy:parsetree_with_tokens(NCypherTD),
                      {ok, {NPTTD, NTTD}}
                  catch _:_ ->
            ...
            %% Test BottomUp
            %% --------------------------------------------------------
            NCypherBU = case ocparse_legacy:parsetree_to_string_bu(ParseTree) of
                            {error, ErrorBU} ->
                                throw({error, ErrorBU});
                            NSBU ->
            ...
            {ok, {NPTreeBU, _NToksBU}}
                = try
                      {ok, {NPTBU, NTBU}} = ocparse_legacy:parsetree_with_tokens(NCypherBU),
                      {ok, {NPTBU, NTBU}}
                  catch _:_ ->
